...
/**
 * Shared helper to assemble complete resume data for a student profile
 */
async function assembleResumeData(whereClause, params) {
  ...
  const personalInfo = {
    ...
    headline: profile.headline || `${degreeName} Candidate - ${deptName}`,
    bio: profile.bio || '',
  ...
  const twelfth = {
    college: profile.twelfth_college || 'Senior Secondary School',
    board: profile.twelfth_board || 'CBSE (Science Stream)',
    passing_year: profile.twelfth_year || '2022',
    percentage: profile.twelfth_percentage || '92.4'
  };

  const tenth = {
    school: profile.tenth_school || 'Secondary School',
    board: profile.tenth_board || 'CBSE Board',
    passing_year: profile.tenth_year || '2020',
    percentage: profile.tenth_percentage || '91.8'
  };
...
/**
 * Get resume data for logged-in student
 */
async function getResumeData(req, res) {
  try {
    const resumeData = await assembleResumeData('sp.user_id = ?', [req.user.id]);
    if (!resumeData) return sendError(res, 'Student profile not found', 404);
...
/**
 * Get resume data by student profile ID (or user ID)
 * Accessible for HR, Institutions, and Public Portfolio Viewers
 */
async function getResumeDataByStudentId(req, res) {
  try {
    const { studentId } = req.params;
    let resumeData = await assembleResumeData('sp.id = ?', [studentId]);
    if (!resumeData) {
      // Try by user_id
      resumeData = await assembleResumeData('sp.user_id = ?', [studentId]);
    }
    if (!resumeData) return sendError(res, 'Candidate profile not found', 404);
    return sendSuccess(res, resumeData, 'Candidate resume data retrieved successfully');
  } catch (error) {
    console.error('[Resume getResumeDataByStudentId Error]', error);
    return sendError(res, 'Failed to fetch candidate resume', 500);
  }
}
